...
#[test]
fn test_render_run_detail_auto_scrolling() {
    let mut app = create_test_app();
    app.active_pane = Pane::RunDetail;
    ...
    for i in 0..50 {
        steps.push(crate::app::StepDetail {
            instance: serde_json::json!({"step_name": format!("step_{}", i), "status": "succeeded"}),
            outputs: vec![],
            history: vec![],
            logs: vec![],
        });
    ...
    let detail = crate::app::WorkflowRunFullDetail {
        detail: WorkflowRunDetail {
            id: RunId::new(Uuid::nil()),
            workflow_name: "long-workflow".to_string(),
            initiating_user: "jacrisp".to_string(),
            status: RunStatus::Succeeded,
            created_at,
            finished_at: None,
    ...
    app.runs.push(crate::app::WorkflowRunDetail {
        id: RunId::new(Uuid::nil()),
        workflow_name: "long-workflow".to_string(),
        initiating_user: "jacrisp".to_string(),
        status: RunStatus::Succeeded,
    ...
    // Select the last step which will be way off-screen
    app.selected_step_index = 49;
    ...
    // The auto-scroll logic inside the render function should have updated the overview_scroll
    // to keep the 50th item in view.
    assert!(
        app.overview_scroll > 0,
        "overview_scroll should automatically update to follow selected step"
    );
}
